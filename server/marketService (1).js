...
function generateBattlePlan(analysis) {
  ...
  else if (passesAllFilters && confidenceScore >= 47) { // Stabilization #3: Deadband - enter at 50, exit at 47
    verdict = 'BUY_NOW';
...
export async function scanMarket(symbols = DEFAULT_STOCKS) {
  ...
  if (!lastScanResults || lastScanResults.length === 0) {
    ...
    if (firestoreCache && firestoreCache.length > 0) {
      lastScanResults = firestoreCache;
  ...
  persistScanCache(sortedResults).catch(() => {});
