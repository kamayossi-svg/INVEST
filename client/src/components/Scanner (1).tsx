...
export default function Scanner({
  ...
  const [searchedStock, setSearchedStock] = useState<StockAnalysis | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setExpandedRow(ticker); // Auto-expand the searched stock
      } else {
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder={isRTL ? 'חפש מניה (לדוגמה: AAPL)' : 'Search ticker (e.g., AAPL)'}
              className="w-full min-h-[44px] px-4 py-2 pl-10 bg-gray-900/80 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              style={{ paddingLeft: isRTL ? '1rem' : '2.5rem', paddingRight: isRTL ? '2.5rem' : '1rem' }}
            />
            <svg
              className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <button
            ...
            className={`min-h-[44px] px-4 py-2 rounded-xl font-medium transition-all active:scale-95 ${
          ...
          {searchedStock && (
            <button
              onClick={clearSearch}
              className="min-h-[44px] px-3 py-2 rounded-xl bg-gray-700 hover:bg-gray-600 text-gray-300 transition-all"
            >
              ✕
            </button>
          )}
      ...
      {/* Searched Stock Result Card */}
      {searchedStock && (
        <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-xl border-2 border-blue-500/50 overflow-hidden">
          <div className="p-4 bg-blue-500/10 border-b border-blue-500/30 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-2xl">🔍</span>
              <div>
                <h3 className="text-white font-bold text-lg">{isRTL ? 'תוצאת חיפוש' : 'Search Result'}</h3>
                <p className="text-blue-300 text-sm">{isRTL ? 'ניתוח מעמיק עבור' : 'Deep analysis for'} {searchedStock.symbol}</p>
              </div>
            </div>
            {/* Earnings Badge for searched stock */}
            {searchedStock.earningsData?.hasUpcomingEarnings && (
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                searchedStock.earningsData.earningsRisk === 'critical'
                  ? 'bg-red-500/20 text-red-400 border border-red-500/30'
                  : searchedStock.earningsData.earningsRisk === 'high'
                  ? 'bg-orange-500/20 text-orange-400 border border-orange-500/30'
                  : 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30'
              }`}>
                <span>📅</span>
                {searchedStock.earningsData.daysUntilEarnings}d
              </div>
            )}
          </div>
          {/* Render searched stock using the same row structure */}
          <table className="w-full">
            <tbody>
              {(() => {
                const stock = searchedStock;
                ...
                return (
                  <>
                    <tr
                      ...
                      className="cursor-pointer hover:bg-gray-800/50 transition-colors border-b border-gray-700/30"
                    >
                      {/* Expand Arrow */}
                      <td className="px-2 md:px-4 py-3 w-8">
                        <svg
                      ...
                      {/* Symbol with earnings badge */}
                      <td className="px-3 md:px-4 py-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-bold text-white text-base md:text-lg">{stock.symbol}</span>
                          {isOwned && (
                            <span className="px-1.5 py-0.5 bg-purple-500/20 text-purple-400 text-xs font-medium rounded border border-purple-500/30">💼</span>
                          )}
                          {stock.earningsData?.hasUpcomingEarnings && (
                            <span className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium ${
                              stock.earningsData.earningsRisk === 'critical'
                                ? 'bg-red-500/20 text-red-400 border border-red-500/30'
                                : stock.earningsData.earningsRisk === 'high'
                                ? 'bg-orange-500/20 text-orange-400 border border-orange-500/30'
                                : 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30'
                            }`}>
                              📅 {stock.earningsData.daysUntilEarnings}d
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {battlePlan.verdict === 'BUY_NOW' && (
                          <button
                            ...
                            className="min-h-[44px] px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-xl text-sm font-semibold transition-all active:scale-95 shadow-lg shadow-green-500/25"
                          >
                            {isRTL ? 'קנה' : 'Buy'}
                          </button>
                        )}
                          ...
                          >
                            ...
                            <td className="px-3 md:px-4 py-3">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {/* Earnings Badge - Calendar Icon with Color Coding */}
                                {stock.earningsData?.hasUpcomingEarnings && (
                                  <span
                                    ...
                                    title={`${isRTL ? 'דוח רווחים בעוד' : 'Earnings in'} ${stock.earningsData.daysUntilEarnings} ${isRTL ? 'ימים' : 'days'} (${stock.earningsData.nextEarningsDate})`}
                                  >
                                    📅 {stock.earningsData.daysUntilEarnings}d
                                  </span>
                                )}
